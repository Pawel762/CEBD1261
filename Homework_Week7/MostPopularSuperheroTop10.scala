...
object MostPopularSuperheroTop10 {
  ...
  def main(args: Array[String]) {
    ...
    val sortedflipped = flipped.sortByKey(false).take(10)
    
        
    //val results = sortedflipped.collect().take(10)
    
    //results.foreach(println)
    
   // I will continue to look at why it's not joining after I finish the other exercises
      
    // Look up the name (lookup returns an array of results, so we need to access the first result with (0)).
    val mostPopularName = namesRdd.lookup(sortedflipped._2)(0)
    
    // Print out our answer!
    println(s"$mostPopularName is the most popular superhero with ${sortedflipped._1} co-appearances.") 
  }
